...
namespace BioBaseCLIA.ScalingQC
{
    public partial class frmQC : frmParent
    {
        ...
        private void btnSaveQC_Click(object sender, EventArgs e)
        {
            ...
            if (!chk12s.Checked &&!chk22s.Checked && !chk10x.Checked  && !chk13s.Checked && !chk41s.Checked)
            {
                frmMsgShow.MessageShow(getString("keywordText.QcManagement"), getString("keywordText.SelectRules"));
                txtOperator.Focus();
                return;
            }
